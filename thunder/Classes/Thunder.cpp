...
void Thunder::newEnemy() {
	...
	for (auto enemy : enemys) {
		...
		if (enemy->getPosition().y < player->getContentSize().height * 1.5) {
			stopAc();
			return;
		}
	}
...
void Thunder::movePlane(char c) {
	// Todo
	if (c == 'A'&&player->getPosition().x > player->getContentSize().width) {
		auto move = MoveBy::create(0.04f,Vec2(-10,0));
		player->runAction(move);
	}
	else if (c == 'D'&&player->getPosition().x < visibleSize.width - player->getContentSize().width) {
		auto move = MoveBy::create(0.04f, Vec2(10, 0));
		player->runAction(move);
	}
}
...
void Thunder::fire() {
	...
	// 移除飞出屏幕外的子弹
	// Todo
	auto moveFire = MoveBy::create(1,Vec2(0,visibleSize.height));
	bullet->runAction(Sequence::create(moveFire, CallFunc::create([&,bullet] {
...
void Thunder::meet(EventCustom * event) {
	...
	for (auto it = bullets.begin(); it != bullets.end();++it) {
		for (auto itenemy = enemys.begin(); itenemy != enemys.end(); ++itenemy) {
			if ((*it)->getBoundingBox().containsPoint((*itenemy)->getPosition())) {
				auto buttle = *it;
				auto enemy = *itenemy;
				enemy->runAction(
					Sequence::create(
						Animate::create(Animation::createWithSpriteFrames(explore,0.05f)),
						CallFunc::create([=] {
				...
				buttle->removeFromParentAndCleanup(true);
				bullets.remove(buttle);
				enemys.remove(enemy);
				flag = true;
				break;
			}
...
void Thunder::addCustomListener() {
	// Todo
	auto buttleListener = EventListenerCustom::create("meet",CC_CALLBACK_1(Thunder::meet,this));

	this->getEventDispatcher()->addEventListenerWithFixedPriority(buttleListener, 1);
}
...
void Thunder::onTouchMoved(Touch *touch, Event *event) {
	...
	if(isClick) {
		...
		if (x > player->getContentSize().width && x < visibleSize.width - player->getContentSize().width) {
			auto moveby = MoveBy::create(0, Vec2(delta.x, 0));
